...
#  ]
#
# If you do not care in which orders fields are checked you can also
...
#
package Symphero::Objects::FilloutForm;
use strict;
use Symphero::Utils;
...
##
# Overrided new method for those who prefer to inheritance style.
#
sub new ($%)
{ my $proto=shift;
  my $class=ref($proto) || $proto;
  my $args=get_args(\@_);
  my $self=Symphero::Objects::Page->new($args);
  bless $self,$class;
...
{ my $self=shift;
  ...
  my $values=$args->{values};
  my @copy;
  foreach my $fdata (ref($fields) eq 'ARRAY' ? @{$fields} : keys %{$fields})
   { my $name;
     if(! ref($fdata))
      { $name=$fdata;
        $fdata=$fields->{$name};
...
dprint "name=$name, values=$values";
...
{ my $self=shift;
  ...
  $fields || throw Symphero::Errors::Page
                   ref($self)." has not set fields for FilloutForm";
  ...
  throw Symphero::Errors::Page ref($self)." unknown field '$name' referred";
}
...
{ my $self=shift;
  ...
  foreach my $fdata (@{$fields})
   { my $name=$fdata->{name};
     ...
     $value=$cgi->param($name) unless defined($value);
     $value=$fdata->{value} unless defined($value) || $fdata->{style} eq 'checkbox';
     my $newerr;
     ...
     $value=~s/^\s*(.*?)\s*$/$1/g;
     $filled++ if length($value) && (!defined($fdata->{phase}) || $fdata->{phase} eq $phase);
     ...
     if(!length($value) && $fdata->{required})
     ...
     elsif($fdata->{minlength} && length($value) && length($value) < $fdata->{minlength})
      { $newerr="is too short!";
      }
     elsif($style eq 'text')
      { # No checks for text
      ...
      { if(length($value) && $fdata->{pair} && $value ne $cgi->param($fdata->{pair}))
      ...
      { eprint ref($self),"::display - Unknown style '$style'";
      }
     ...
     if($style eq 'country')
      ...
      { my @cl=qw(January February March April May June July
        ...
        for(my $i=0; $i!=12; $i++)
         { my $sel=($value && $value == $i+1) ? " SELECTED" : "";
           $html.=sprintf("<OPTION$sel>%02u - %s</OPTION>\n",$i+1,$cl[$i]);
         }
         ...
         { my $minyear=$self->calculate_year($fdata->{minyear});
           ...
           for(my $i=$minyear; $i<=$maxyear; $i++)
            { my $sel=($value && $value == $i) ? " SELECTED" : "";
              $html.=sprintf("<OPTION$sel>%04u</OPTION>\n",$i);
            }
      ...
      { $fdata->{html}='<INPUT TYPE="CHECKBOX" NAME="' .
                       t2hf($name) . '"' .
                       ((defined($fdata->{value}) ? $fdata->{value} : $value) ? ' CHECKED' : '') . '>';
      }
  ...
  ##
  # If there were errors - displaying form.
  #
  my $obj=$self->object;
  if(!$filled || $errstr)
